...
describe('JSDomEnvironment', () => {
  ...
  it('should respect userAgent option', () => {
    ...
    expect(env.global.window.navigator.userAgent).toBe('foo');
  });
  ...
  it('should respect url option', () => {
    ...
    expect(env.global.window.location.href).toBe('http://localhost/');
    ...
    expect(envWithUrl.global.window.location.href).toBe('https://jestjs.io/');
  });
